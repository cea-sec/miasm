...
uint64_t get_mem_base_addr(vm_mngr_t *vm_mngr, uint64_t addr,
			   uint64_t *addr_base);
unsigned int MEM_LOOKUP(vm_mngr_t *vm_mngr, unsigned int my_size,
			uint64_t addr);
...
int vm_read_mem(vm_mngr_t *vm_mngr, uint64_t addr, char **buffer_ptr,
		uint64_t size);
int vm_write_mem(vm_mngr_t *vm_mngr, uint64_t addr, char *buffer,
		 uint64_t size);
...
#define UDIV(sizeA)                                                            \
	uint##sizeA##_t udiv##sizeA(vm_cpu_t *vmcpu, uint##sizeA##_t a,        \
				    uint##sizeA##_t b)                         \
	{                                                                      \
		uint##sizeA##_t r;                                             \
		if (b == 0) {                                                  \
...
#define UMOD(sizeA)                                                            \
	uint##sizeA##_t umod##sizeA(vm_cpu_t *vmcpu, uint##sizeA##_t a,        \
				    uint##sizeA##_t b)                         \
	{                                                                      \
		uint##sizeA##_t r;                                             \
		if (b == 0) {                                                  \
...
#define IDIV(sizeA)                                                            \
	int##sizeA##_t idiv##sizeA(vm_cpu_t *vmcpu, int##sizeA##_t a,          \
				   int##sizeA##_t b)                           \
	{                                                                      \
		int##sizeA##_t r;                                              \
		if (b == 0) {                                                  \
...
#define IMOD(sizeA)                                                            \
	int##sizeA##_t imod##sizeA(vm_cpu_t *vmcpu, int##sizeA##_t a,          \
				   int##sizeA##_t b)                           \
	{                                                                      \
		int##sizeA##_t r;                                              \
		if (b == 0) {                                                  \
...
struct memory_page_node *
create_memory_page_node(uint64_t ad, unsigned int size,
			unsigned int access); // memory_page* mp);
