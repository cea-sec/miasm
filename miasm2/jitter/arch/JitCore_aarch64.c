...
PyObject *cpu_set_gpreg(JitCpu *self, PyObject *args)
{
	...
	while (PyDict_Next(dict, &pos, &d_key, &d_value)) {
		...
		for (i = 0; i < sizeof(gpreg_dict) / sizeof(reg_dict); i++) {
			if (strcmp(PyString_AsString(d_key),
				   gpreg_dict[i].name))
				continue;
			*((uint32_t *)(((char *)(self->cpu)) +
				       gpreg_dict[i].offset)) = val;
			found = 1;
			break;
		}
...
{
	printf("X0  %.16" PRIX64 " X1  %.16" PRIX64 " X2  %.16" PRIX64
	       " X3  %.16" PRIX64 " "
	       "X4  %.16" PRIX64 " X5  %.16" PRIX64 " X6  %.16" PRIX64
	       " X7  %.16" PRIX64 "\n",
	       vmcpu->X0, vmcpu->X1, vmcpu->X2, vmcpu->X3, vmcpu->X4, vmcpu->X5,
	       vmcpu->X6, vmcpu->X7);
	printf("X8  %.16" PRIX64 " X9  %.16" PRIX64 " X10 %.16" PRIX64
	       " X11 %.16" PRIX64 " "
	       "X12 %.16" PRIX64 " X13 %.16" PRIX64 " X14 %.16" PRIX64
	       " X15 %.16" PRIX64 "\n",
	       vmcpu->X8, vmcpu->X9, vmcpu->X10, vmcpu->X11, vmcpu->X12,
	       vmcpu->X13, vmcpu->X14, vmcpu->X15);
	printf("X16 %.16" PRIX64 " X17 %.16" PRIX64 " X18 %.16" PRIX64
	       " X19 %.16" PRIX64 " "
	       "X20 %.16" PRIX64 " X21 %.16" PRIX64 " X22 %.16" PRIX64
	       " X23 %.16" PRIX64 "\n",
	       vmcpu->X16, vmcpu->X17, vmcpu->X18, vmcpu->X19, vmcpu->X20,
	       vmcpu->X21, vmcpu->X22, vmcpu->X23);
	printf("X24 %.16" PRIX64 " X25 %.16" PRIX64 " X26 %.16" PRIX64
	       " X27 %.16" PRIX64 " "
	       "X28 %.16" PRIX64 " X29 %.16" PRIX64 " LR  %.16" PRIX64 "\n",
	       vmcpu->X24, vmcpu->X25, vmcpu->X26, vmcpu->X27, vmcpu->X28,
	       vmcpu->X29, vmcpu->LR);

	printf("SP  %.16" PRIX64 " PC  %.16" PRIX64 " "
	       "zf  %.16" PRIX32 " nf  %.16" PRIX32 " of  %.16" PRIX32
	       " cf  %.16" PRIX32 "\n",
...
PyObject *cpu_get_exception(JitCpu *self, PyObject *args)
{
	return PyLong_FromUnsignedLongLong(
	    (uint64_t)(((vm_cpu_t *)self->cpu)->exception_flags));
}
...
void check_automod(JitCpu *jitcpu, uint64_t addr, uint64_t size)
{
	...
	if (!(((VmMngr *)jitcpu->pyvm)->vm_mngr.exception_flags &
	      EXCEPT_CODE_AUTOMOD))
		return;
	result =
	    PyObject_CallMethod(jitcpu->jitter, "automod_cb", "LL", addr, size);
	Py_DECREF(result);
}
...
PyObject *vm_set_mem(JitCpu *self, PyObject *args)
{
	...
	ret = vm_write_mem(&(((VmMngr *)self->pyvm)->vm_mngr), addr, buffer,
			   size);
	if (ret < 0)
		RAISE(PyExc_TypeError, "arg must be str");
...
static PyGetSetDef JitCpu_getseters[] = {
    {"vmmngr", (getter)JitCpu_get_vmmngr, (setter)JitCpu_set_vmmngr, "vmmngr",
     NULL},

    {"jitter", (getter)JitCpu_get_jitter, (setter)JitCpu_set_jitter, "jitter",
     NULL},
...
PyMODINIT_FUNC initJitCore_aarch64(void)
{
	...
	JitCore_aarch64_Error =
	    PyErr_NewException("JitCore_aarch64.error", NULL, NULL);
