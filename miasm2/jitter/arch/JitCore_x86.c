...
reg_dict gpreg_dict[] = {
    {.name = "RAX", .offset = offsetof(vm_cpu_t, RAX)},
...
void dump_gpregs(vm_cpu_t *vmcpu)
{
	...
	printf("RAX %.16" PRIX64 " RBX %.16" PRIX64 " RCX %.16" PRIX64
	       " RDX %.16" PRIX64 "\n",
	       vmcpu->RAX, vmcpu->RBX, vmcpu->RCX, vmcpu->RDX);
	printf("RSI %.16" PRIX64 " RDI %.16" PRIX64 " RSP %.16" PRIX64
	       " RBP %.16" PRIX64 "\n",
	       vmcpu->RSI, vmcpu->RDI, vmcpu->RSP, vmcpu->RBP);
	printf("zf %.16" PRIX64 " nf %.16" PRIX64 " of %.16" PRIX64
	       " cf %.16" PRIX64 "\n",
...
{
	return PyLong_FromUnsignedLongLong(
	    (uint64_t)(((vm_cpu_t *)self->cpu)->interrupt_num));
}
...
PyObject *cpu_get_segm_base(JitCpu *self, PyObject *args)
{
	...
	PyGetInt(item1, segm_num);
	v = PyInt_FromLong(
	    (long)(((vm_cpu_t *)self->cpu)->segm_base[segm_num]));
	return v;
}
...
static PyMethodDef JitCpu_methods[] = {
    ...
    {"get_segm_base", (PyCFunction)cpu_get_segm_base, METH_VARARGS, "X"},
    {"set_segm_base", (PyCFunction)cpu_set_segm_base, METH_VARARGS, "X"},
...
#define getset_reg_E_u32(regname)                                              \
	...
	static int JitCpu_set_E##regname(JitCpu *self, PyObject *value,        \
					 void *closure)                        \
	{                                                                      \
		uint64_t val;                                                  \
		PyGetInt_ret0(value, val);                                     \
		val &= 0xFFFFFFFF;                                             \
		val |= ((vm_cpu_t *)(self->cpu))->R##regname &                 \
...
#define getset_reg_R_u16(regname)                                              \
	...
	static int JitCpu_set_##regname(JitCpu *self, PyObject *value,         \
					void *closure)                         \
	{                                                                      \
		uint64_t val;                                                  \
		PyGetInt_ret0(value, val);                                     \
